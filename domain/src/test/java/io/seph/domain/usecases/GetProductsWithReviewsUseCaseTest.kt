...
class GetProductsWithReviewsUseCaseTest : BaseTest() {
    ...
    fun `should return ProductsWithReviewModel when fetching products and reviews successfully`() =
        runTest {
            every { mockRepository.getProducts() } returns mockProducts
            every { mockReviewsRepository.getProductsReviews() } returns mockReviews
            ...
            val result = getProductsWithReviewsUseCase.execute()

            verify(exactly = 1) { mockRepository.getProducts() }
            verify(exactly = 1) { mockReviewsRepository.getProductsReviews() }
    ...
    fun `should return error when fetching products fails`() = runTest {
        ...
        every { mockRepository.getProducts() } throws exception
        val result = getProductsWithReviewsUseCase.execute()
        verify(exactly = 1) { mockRepository.getProducts() }
        verify(exactly = 0) { mockReviewsRepository.getProductsReviews() }
    ...
    fun `should return error when fetching reviews fails`() = runTest {
        val exception = IllegalStateException()
        every { mockRepository.getProducts() } returns mockk()
        every { mockReviewsRepository.getProductsReviews() } throws exception
